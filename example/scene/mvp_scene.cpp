...
void MVPScene::OnKeyHold(pav::KeyCode keycode)
{
	...
	case pav::KEY_J:
		...
		break;
	case pav::KEY_O:
		if (go_player_client_transform_->GetPosition().x >= go_player_client_transform_2->GetPosition().x && isP2Tagged
		...
		break;
	case pav::KEY_F:
		if (go_player_client_transform_2->GetPosition().x >= go_player_client_transform_->GetPosition().x && isP1Tagged
